    ...
    private static String encodeULID(byte[] part1, byte[] part2) {
        ...
        for (int i = length - 1; i >= 0; i--) {
            ...
            for (int j = 0; j < 5; j++) {
                pos = (i * 5 + j);
                inpos = pos & 0x7;
                bpos = (pos >> 3) - ((pos >= part2.length * 8) ? part2.length : 0);
                value = (value << 1) + (((which(part1, part2, pos)[bpos] & (0x1 << inpos)) != 0) ?   1 : 0);
//                System.out.printf("value1=%d",value);
            }
...
//            String ulid = ULIDGenerator.genULID();
//            s.add(ulid);
...
//        System.out.println("time spend--"+(System.currentTimeMillis()-st));
        ...
        try {
            FileWriter writer = new FileWriter(".\\test.txt");
            BufferedWriter bw = new BufferedWriter(writer);
            int size = 1;
            Scanner scanner = new Scanner(System.in);
            System.out.printf("\n  input the number of cdkeys: ");
