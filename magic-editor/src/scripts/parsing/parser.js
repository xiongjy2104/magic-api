...
import RequestParameter from '../editor/request-parameter.js'
...
const binaryOperatorPrecedence = [
	...
	[TokenType.Less, TokenType.LessEqual, TokenType.Greater, TokenType.GreaterEqual, TokenType.InstanceOf],
...
export class Parser {
	...
	matchTypeDefine() {
		...
		let index = this.stream.makeIndex();
		try {
			if ('new' === this.stream.getPrev().getText()) {
				return false;
			}
			if (this.stream.match(TokenType.Identifier, false)) {
				return true;
			}
			let end = this.stream.getPrev().getSpan().getEnd();
	...
	parseVarDefine() {
		...
		if ( (isMapAccess = this.stream.match(TokenType.LeftCurly, false)) || this.stream.match(TokenType.LeftBracket, false) ) {
			...
			return new VarDestructuringDefine(new Span(opening, this.stream.getPrev().getSpan()), tokens, this.parseExpression(), opening.getText(), isMapAccess)
		}
		let token = this.stream.expect(TokenType.Identifier);
			...
			while (this.stream.hasMore() && this.stream.match([TokenType.LeftParantheses, TokenType.LeftBracket, TokenType.Period, TokenType.QuestionPeriod, TokenType.ColonColon], false)) {
				...
				// field or method access
					...
					if (this.linqLevel > 0 && this.stream.match(TokenType.Asterisk, false)) {
						target = new MemberAccess(target.getSpan(), optional, this.stream.expect(TokenType.Asterisk).getSpan(), true);
					} else {
...
const objToMap = obj => {
	const map = {}
	obj?.children?.forEach(it => map[it.name] = it)
	return map
}
function processBody(body, srcObj) {
	let arr = []
	let defaultParam = {
		name: '',
		value: '',
	...
	if (Array.isArray(body)) {
		...
		Object.keys(body).forEach(name => {
			...
			let param = {
				...
				required: srcObj[name]?.required === true,
				validateType: srcObj[name]?.validateType || '',
